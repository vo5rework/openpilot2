...
class ACCController:
  ...
  def __init__(self, press_cooldown_frames: int = 40, human_guard_frames: int = 300, auto_guard_frames: int = 40) -> None:
  ...
  def update(self, CS, *, lat_active: bool, frame: int) -> Tuple[bool, int]:
    ...
    # Unity behavior: if target is below stock cruise floor, cancel instead of spamming decel.
    if desired_speed_ms < self.MIN_CRUISE_SPEED_MS and current_speed_ms > 0.0:
    ...
    if speed_offset_kph < -2.0 * full_step_kph and current_speed_ms > 0.0:
      return True, int(CruiseButtons.CANCEL)
